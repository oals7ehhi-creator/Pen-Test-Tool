...
- Audit non-null-chain test (SI-026): two events with the same `seq` on a tenant or global chain are REJECTED (the NULL-key uniqueness bug is fixed by `chain_id`); each chain verifies independently.
...
- Budget/time-interlock invariant (SI-017/SI-062): budget uses an identifiable reservation ledger with idempotent commit/release and crash-expiry (no leak, no double-spend, no strand); window/expiry/e-stop re-checked at JIT mint and Stage 2; a spec past its window gets no grant.
