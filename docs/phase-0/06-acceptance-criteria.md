...
- Every outbound target request is authorized via the two-stage flow (JIT Scope Authority Stage-1 grant over the immutable spec + Guarded Egress Broker Stage-2 reconstruction/IP-validation/pinning); grants are single-use and audience-bound; the request budget uses **reserve-at-mint / commit-on-send / release-on-denial**; window/expiry/e-stop are re-checked at JIT mint and Stage 2; WebSocket handshakes are scoped/pinned like HTTP and established connections are bounded and terminated on interlocks; authorization attestation and scope expansion require dual control (≥2 role-verified approvers, SoD, per-decision hash binding); scope-breadth limits (min CIDR prefix, IPv4-equivalent-address & CIDR-entry ceilings, wildcard/host gating; IPv6 by prefix floor) and scope expansion are gated by elevated dual approval.
...
- Budget reserve/commit/release tests: total *sent* never exceeds request_budget_total under concurrency (boundary total-1/total/total+1); killing a worker after reserve but before send releases the unit (no leak); a pre-send denial releases the reservation.
...
- Budget/time-interlock invariant (SI-017/SI-062): budget is reserve/commit/release with no leak or double-spend; window/expiry/e-stop re-checked at JIT mint and Stage 2; a spec past its window gets no grant.
- WebSocket invariant (SI-063): handshakes scoped/pinned like HTTP; established connections bounded and terminated on e-stop/window/expiry.
...
- Worker isolation and network segmentation are enforced; network-layer egress filtering defaults to deny and permits only engagement-approved targets plus required infrastructure.
