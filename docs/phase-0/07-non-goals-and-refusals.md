...
| F3 | **No third-party / shared infrastructure targeting** (CDNs, payment processors, SSO/identity providers, cloud metadata, SaaS APIs) unless individually allowlisted with authorization. | We have no right to test infrastructure the client does not own or control. |
| F4 | **No SSRF pivoting to cloud metadata, RFC1918, link-local, or localhost.** | These are the DNS-rebinding / SSRF targets the platform is explicitly built to *block*, not reach. |
...
| "Fire the SSRF at `169.254.169.254` / the internal service." | No reaching metadata, private, link-local, or localhost ranges. | **Approved controlled-callback (canary) infrastructure only**: an inert out-of-band token confirms the SSRF primitive (blind indicator) without touching any real internal target. |
