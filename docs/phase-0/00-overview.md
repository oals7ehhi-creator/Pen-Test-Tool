...
| **Budget reservation ledger** | Budget is an identifiable `budget_reservation` per grant `jti` with idempotent commit/release and crash-expiry — the sent count never exceeds the total, and a crashed worker's reservation auto-expires instead of stranding. |
