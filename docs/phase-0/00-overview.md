...
| 5 | Safety invariants — **63 absolute, test-enforced invariants (SI-###)** | `05-safety-invariants.md` |
...
| **`request_spec`** | The **immutable, fully-hashed** object placed on the queue: the exact request (method, canonical URL/host/port/scheme/path, fixed header-set id, inert payload id, session reference, mode, approval ref) hashed as `spec_sha256`. Created once, never mutated; the queue stores only `(spec_id, tenant_id)`. |
...
| **Budget reserve/commit/release** | A budget unit is reserved at JIT grant-mint, committed on send, released on any pre-send denial — the sent count never exceeds the total and no unit leaks. |
