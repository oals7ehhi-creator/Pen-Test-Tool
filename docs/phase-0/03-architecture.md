...
- **ADR-6: Row-Level Security for tenant isolation, plus composite-key engagement isolation for authority references.** Rationale: tenant isolation is enforced at the datastore via RLS (surviving application bugs; cross-tenant leakage requires actively removing a policy, CI-guarded), and every *security-authority* reference additionally carries `engagement_id` in a composite `(id, tenant_id, engagement_id)` foreign key (`04` §1), so a request can never bind an authorization, approval, scope version, session, or budget lease from another engagement even within the same tenant and with an identical `scope_hash`. Trade-off: RLS discipline in every query path plus engagement-bearing keys on authority tables; enforced by tests (Phase 10 tenant- and engagement-isolation tests).
...
- **Tenant & engagement isolation:** RLS tests prove no cross-tenant read/write, and composite `(id, tenant_id, engagement_id)` authority FKs plus the Stage-1 engagement-equality check prove no cross-engagement authority binding within a tenant (Phase 10).
