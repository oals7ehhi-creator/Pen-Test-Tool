...
**52 absolute, machine-checkable safety invariants (SI-###).** Each is a property that automated tests MUST enforce and that the release pipeline MUST fail on if violated (NFR + Phase 10 gate). Invariants SI-041–SI-052 were added during the Phase 0 adversarial design review (see `08-design-review-and-critique-resolution.md`).
...
|---|---|---|
| SI-001 | Every outbound network request originating from any worker, check module, or tool adapter MUST pass through the single ScopeGuard … | scope escape, SSRF |
...
| SI-003 | The IP address the ScopeGuard client actually connects to MUST be identical to the IP that was validated in the ALLOW verdict; the… | SSRF, scope escape |
...
| SI-005 | Every HTTP redirect (any 3xx with a Location, plus meta-refresh and equivalent) MUST be re-submitted to the ScopeGuard and re-vali… | scope escape, SSRF |
| SI-006 | The following ranges MUST NEVER be contacted unless that exact range is explicitly present in the engagement's scope allowlist: lo… | SSRF, scope escape |
...
| SI-033 | Worker network egress at the infrastructure layer MUST be default-deny and dynamically restricted to exactly the destination set c… | scope escape, SSRF, unsafe plugin execution |
...
| SI-043 | The scope decision token MUST additionally bind the canonical path-prefix and (where policy restricts it) the HTTP method, so an I… | scope escape |
| SI-044 | The forbidden-range classifier MUST decode IPv4-mapped, IPv4-compatible, 6to4 (2002::/16), Teredo (2001::/32), and NAT64 (64:ff9b:… | SSRF |
...
| SI-047 | Authorization attestation, and any scope expansion that introduces a new host, domain, or IP range, MUST require two distinct appr… | scope escape |
...
**Every outbound network request originating from any worker, check module, or tool adapter MUST pass through the single ScopeGuard egress client and MUST carry a decision token proving an ALLOW verdict bound to (engagementId, authorizationId, canonicalTarget, resolvedIP, port, protocol); no request executes without a fresh, cryptographically-bound ALLOW token.**

- **Rationale.** A single mandatory choke point is the only way to make scope enforcement machine-checkable. If any code path can open a socket directly, every other invariant becomes advisory. The decision token binds the exact request that was authorized to the exact request that is sent, closing the check-vs-use gap.
- **Enforcement point.** ScopeGuard egress client (the ONLY component permitted to create sockets/HTTP connections); enforced in the worker runtime plus an architectural/static-analysis CI gate that forbids raw socket, http.client, requests, urllib, curl, or DNS libraries anywhere outside the ScopeGuard module.
- **Test approach.** Architectural test: static analysis (import-linter / dependency-cruiser / AST scan) fails CI if any module besides ScopeGuard references a socket-capable API. Integration test: attempt to issue requests via every check module and tool adapter with a forged/absent token and assert the connection is refused before any TCP SYN (verified with an in-process socket monkeypatch counter that asserts zero sockets opened). Property-based test: for random engagement/target inputs, no socket opens unless a matching valid token exists.
...
- **Enforcement point.** ScopeGuard verdict function default branch (returns DENY on any non-affirmative state); scheduler refuses to enqueue jobs for engagements lacking a validated scope record.
...
**The IP address the ScopeGuard client actually connects to MUST be identical to the IP that was validated in the ALLOW verdict; the connection MUST be pinned to that validated IP and MUST NOT be re-resolved by the OS resolver or HTTP stack between validation and connect.**

- **Rationale.** DNS-rebinding protection. Validating a hostname then letting the socket layer re-resolve permits a TOCTOU attack where the second resolution returns an in-scope-bypassing IP (e.g., 169.254.169.254). Pinning the socket to the exact validated IP eliminates the window.
- **Enforcement point.** ScopeGuard egress client custom connection factory: resolves once, validates all records, then dials the pinned in-scope IP directly and sets the SNI/Host header to the original hostname.
- **Test approach.** Integration test with a controllable DNS server whose response changes between resolution calls (first in-scope, second 169.254.169.254). Assert the actual TCP connection goes only to the first, validated IP. Test with TTL=0 and multiple rapid requests. Assert the resolver is invoked exactly once per request and the connected peer address equals the validated address.
- **Violation impact.** SSRF to cloud metadata or internal services via rebinding while appearing in-scope — a primary bypass of the entire scope model.
...
- **Enforcement point.** ScopeGuard resolution-validation step (validates the full record set, not the first usable entry).
...
**Every HTTP redirect (any 3xx with a Location, plus meta-refresh and equivalent) MUST be re-submitted to the ScopeGuard and re-validated (including fresh DNS resolution and IP-range checks); a redirect whose target is out-of-scope MUST NOT be followed, and cross-scope redirect chains terminate immediately.**
...
- **Enforcement point.** ScopeGuard egress client redirect handler (auto-follow disabled at the raw client; redirects handled explicitly with re-validation and a bounded hop count).
...
**The following ranges MUST NEVER be contacted unless that exact range is explicitly present in the engagement's scope allowlist: loopback (127.0.0.0/8, ::1), RFC1918 (10/8, 172.16/12, 192.168/16), link-local (169.254.0.0/16, fe80::/10), cloud metadata (169.254.169.254, fd00:ec2::254, and known provider metadata endpoints), unique-local IPv6 (fc00::/7), CGNAT (100.64.0.0/10), 0.0.0.0/8, 192.0.0.0/24, 192.0.2.0/24, 198.18.0.0/15, multicast/broadcast/reserved (224.0.0.0/4, 240.0.0.0/4, 255.255.255.255), and IPv4-mapped/compatible forms of any of these.**

- **Rationale.** These ranges are where SSRF does damage (metadata credential theft, internal service access). Blocking them by default, requiring explicit opt-in per exact range, contains SSRF even when a hostname resolves into them.
- **Enforcement point.** ScopeGuard IP-classification module applied to every resolved IP and every redirect/connect target, before the allowlist is even consulted (deny ranges are checked against explicit allow entries only).
- **Test approach.** Exhaustive table-driven unit tests covering each range including boundary addresses (network/broadcast/first/last), IPv4-mapped IPv6 (::ffff:169.254.169.254), IPv4-compatible, and all metadata provider IPs. Property-based test generating random addresses across the full IPv4/IPv6 space asserting classification correctness. Integration test proving a request to each range is refused unless the exact CIDR is allowlisted.
...
- **Enforcement point.** ScopeGuard canonicalization layer, run before both allowlist matching and forbidden-range classification.
- **Test approach.** Parser/URL fuzzing corpus of known SSRF bypass encodings asserting each normalizes to the correct canonical target or is rejected. Property-based round-trip tests. Differential test against a reference URL parser to catch parser-confusion (frontend parses one host, ScopeGuard another).
...
- **Enforcement point.** ScopeGuard verdict precedence order: exclusion check evaluated last with veto power over any allow.
...
- **Enforcement point.** ScopeGuard scheme/port allowlist check; the egress client only implements HTTP/HTTPS transports.
...
- **Enforcement point.** ScopeGuard path-matching step operating on canonicalized paths.
...
- **Enforcement point.** ScopeGuard validates authorization freshness on every request (defense in depth); scheduler/worker supervisor runs an expiry sweep that cancels queued jobs and signals in-flight abort; auth expiry re-checked at dequeue and at connect time.
...
- **Enforcement point.** Scheduler window-gate at enqueue/dequeue plus ScopeGuard per-request window check using the engagement timezone.
...
- **Enforcement point.** Emergency-stop flag checked by ScopeGuard on every request AND by the worker supervisor which sends abort signals; flag stored in a fast, consistent store read on the hot path.
...
- **Enforcement point.** ScopeGuard/worker instrumentation feeding a per-engagement and per-target breaker evaluated before each request.
...
- **Enforcement point.** Distributed concurrency limiter (per-engagement leased slots in a shared store) consulted by ScopeGuard before every request; slots released on completion/abort with leak-safe timeouts.
...
- **Enforcement point.** Distributed token-bucket/rate limiter per engagement enforced in ScopeGuard on the send path.
...
- **Enforcement point.** ScopeGuard decrements a shared budget counter atomically per request; scheduler stops on zero.
...
- **Enforcement point.** Validation executor gate that recomputes the plan hash at execution time and matches it to an immutable approval record before invoking ScopeGuard; ScopeGuard additionally requires an approval reference in the decision token for intrusive-class requests.
...
- **Enforcement point.** Check-engine payload factory restricted to an allowlisted, versioned payload catalog; an egress payload scanner in ScopeGuard rejects any outbound body/param matching destructive-signature deny rules as a backstop.
- **Test approach.** Unit test asserting every payload in every check's catalog is inert per a destructive-signature classifier. Property-based test attempting to coerce checks into emitting destructive strings via crafted target metadata, asserting none escape. Backstop test: inject a synthetic destructive payload into the send path and assert ScopeGuard blocks it.
...
**The audit trail MUST be append-only and tamper-evident (hash-chained/sequence-linked so any insertion, deletion, or modification is detectable), and every safety-relevant action — scope decision, request issued/blocked, approval, e-stop, expiry halt, config change, login, data export — MUST produce exactly one corresponding immutable audit event.**

- **Rationale.** Tamper-evidence and completeness are prerequisites for trusting that the other invariants held in production and for post-incident reconstruction.
- **Enforcement point.** Audit service with hash-chained events (each event references the prior event's hash); write-once storage semantics; instrumentation hooks at every safety-relevant action site.
- **Test approach.** Integration test asserting each safety-relevant action emits exactly one event with correct linkage. Tamper test: modify/delete/reorder a stored event and assert chain verification fails. Coverage test enumerating action types and asserting none is unlogged. Property-based test on chain verification.
- **Violation impact.** Undetectable tampering and inability to prove authorized, in-scope operation — destroys accountability and the tamper-evidence guarantee.
...
- **Enforcement point.** ScopeGuard and the validation executor treat a durable audit write as a precondition (write-then-act ordering) rather than fire-and-forget.
...
- **Enforcement point.** Tool-adapter parsing layer using safe structured parsers (no code-eval, no unsafe deserialization), schema validation, and a hard boundary that raw tool output cannot expand scope or spawn requests without going through ScopeGuard and (for intrusive follow-ups) approval.
...
**External tools and plugins MUST run in isolated containers with default-deny network egress; their only permitted network destinations are those approved by the ScopeGuard for the current engagement, enforced at the network/sandbox layer (not merely inside the tool), and they run under CPU/memory/time/request limits.**
...
**Worker network egress at the infrastructure layer MUST be default-deny and dynamically restricted to exactly the destination set currently approved by the ScopeGuard for that worker's active engagement(s); a worker cannot open a connection to any host the ScopeGuard has not authorized, independent of application-layer checks.**

- **Rationale.** Defense in depth: even if an application-layer bypass existed, a network-layer egress lock confines the worker to authorized destinations, containing SSRF and scope escape.
- **Enforcement point.** Network egress policy / firewall / sidecar proxy fed by the ScopeGuard verdict set; workers have no direct route to the internet except through the guarded path.
- **Test approach.** Container/network isolation test: from a worker, bypass the application client and attempt raw connections to out-of-scope, private, and metadata addresses, asserting the network layer blocks them. Assert the egress allowlist updates when scope changes and revokes on expiry/e-stop. Chaos test removing scope and asserting egress collapses to deny-all.
- **Violation impact.** Application-layer bypass reaching unauthorized hosts because the network layer trusted the app — loss of defense in depth.
...
- **Enforcement point.** Check engine draws callback targets only from the engagement's approved callback-infra registry; ScopeGuard/ egress policy blocks any other callback destination.
...
- **Enforcement point.** Crawler and correlation layers write discoveries to an inventory store flagged in/out of current scope; ScopeGuard evaluates every request against the operator-defined scope only, never against auto-discovered additions.
...
**The scope decision token MUST additionally bind the canonical path-prefix and (where policy restricts it) the HTTP method, so an IP-level ALLOW cannot be replayed against out-of-scope paths or verbs.**

- **Rationale.** A token bound only to (engagement, authz, host, IP, port, protocol) authorizes any path and any method on that host — combined with the CONNECT blind spot this defeats path/method scoping.
- **Enforcement point.** Scope Authority (token minting) + Guarded Egress Broker / adapter (token verification against the actual request line).
- **Test approach.** Property test: a token minted for GET /app cannot be replayed for POST /admin on the same IP; the broker/adapter rejects mismatched path/method.
- **Violation impact.** Path- and method-level scope escape via token replay.
- **Related threats.** scope escape

### SI-044

**The forbidden-range classifier MUST decode IPv4-mapped, IPv4-compatible, 6to4 (2002::/16), Teredo (2001::/32), and NAT64 (64:ff9b::/96) forms and re-classify the embedded IPv4 against all forbidden ranges. Cloud-metadata addresses are reachable ONLY via an exact-/32 (IPv4) or /128 (IPv6) elevated allow entry, never via a covering CIDR.**

- **Rationale.** Transition IPv6 forms can encode 169.254.169.254 or RFC1918 and slip past a naive deny list; a covering /16 allow must never silently grant the metadata address.
- **Enforcement point.** Scope Authority address classifier + Guarded Egress Broker resolved-IP check.
- **Test approach.** Table-driven test over 6to4/Teredo/NAT64/IPv4-mapped forms (including 2002:a9fe:a9fe::) asserting the decoded IPv4 is classified forbidden; test that allowlisting 169.254.0.0/16 does NOT permit 169.254.169.254.
- **Violation impact.** SSRF to cloud metadata / internal ranges via encoded addresses.
...
**Authorization attestation, and any scope expansion that introduces a new host, domain, or IP range, MUST require two distinct approvers (dual control), neither of whom is the executing tester, with the authorization document's sha256 pinned at both approvals. A single user MUST NOT hold two separation-of-duties-conflicting roles on the same engagement, even if those roles are granted globally.**

- **Rationale.** The primary abuse actor is a privileged insider; scope-hash binding stops silent drift but not a deliberate broaden-and-re-attest by one person. The legal gate the whole product rests on needs a two-person rule.
- **Enforcement point.** Backend API approval state machine + RBAC/SoD constraints; audit hash-chain records both approvals.
- **Test approach.** Authorization tests: a single actor cannot attest+expand+approve; SoD test that conflicting roles cannot both be exercised by one user on one engagement; assert document_sha256 pinned at each approval.
