...
**Every security/request-context reference in a `request_spec` (check, tool template, header-set, payload, WebSocket frame-set) MUST be an immutable content digest into `catalog_template`, and ALL such digests MUST be folded into `spec_sha256`; catalog templates are immutable and content-addressed, and each digest FK enforces the referenced template's KIND; the non-secret session identity is bound via `session_digest` (`sha256(account_id, session_version)`) and query values via a protected `query_value_digest`, both folded into `spec_sha256` (the raw secret and query values are injected at the broker). A `request_spec` MUST be repeatable across jobs/runs — `spec_sha256` is a reusable content digest that excludes `run_id`/`job_id`/`session_ref` and carries no uniqueness constraint — and a repeat is a distinct instance with its own single-use grant, not a replay.**
